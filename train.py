...
from config import *
import config
from data_state import get_data_loader
...
if __name__ == '__main__':
    ...
    model = Model(num_words, num_classes, word2vec=word2vec,hid_dim=hid_dim, freeze=freeze,step=2,lstm_layer=2,ettention=config.attention)
    ...
    for epoch in range(start + 1, 300):
        ...
        if best_acc < test_acc:
            best_acc = test_acc
            if config.attention:
                torch.save(model, "./model_save/train_1_model_save.pt")
